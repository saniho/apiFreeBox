...
try:
	...
	from . import fw
	from . import phone
	from . import airmedia
	from . import freeplugs
	from . import freeplayer
except:
	import exceptions
	import access
	import system
...
class freepybox:
    def __init__(self, app_desc=app_desc, token_file=token_file, api_version='v6', timeout=10):
    ...
    def open(self, host, port=80):
        ...
        '''
        if not self._is_app_desc_valid(self.app_desc): raise exceptions.InvalidTokenError('invalid application descriptor')
        ...
        self.system = system.System(self._access)
        self.connection = connection.Connection(self._access)
        self.dhcp = dhcp.Dhcp(self._access)
        self.switch = switch.Switch(self._access)
        self.lan = lan.Lan(self._access)
        self.wifi = wifi.Wifi(self._access)
        self.fs = fs.Fs(self._access)
        self.call = call.Call(self._access)
        self.fsnav = fsnav.Fsnav(self._access)
        self.fw = fw.Fw(self._access)
        self.phone = phone.Phone(self._access)
        self.airmedia = airmedia.Airmedia(self._access)
        self.freeplugs = freeplugs.Freeplugs(self._access)
        self.freeplayer = freeplayer.Freeplayer(self._access)
    ...
    def close(self):
        ...
        '''
        if self._access is None: raise exceptions.NotOpenError('Freebox is Not opened')
    ...
    def _get_freebox_access(self, host, port, api_version, token_file, app_desc, timeout=10):
        ...
        if app_token is None or file_app_desc != app_desc:
                ...
                while(status != 'granted'):
                    ...
                    if status == 'denied':
                        raise exceptions.AuthorizationError('the app_token is invalid or has been revoked')
                    ...
                    # timeout = authorization failed
                        ...
                        raise exceptions.AuthorizationError('timeout')
        ...
        fbx_access = access.Access(self.session, base_url, session_token, timeout)
    ...
    def _get_app_token(self, base_url, app_desc, timeout=10):
        ...
        if not resp.get('success'):
            raise exceptions.AuthorizationError('authentification failed')
    ...
    def _get_session_token(self, base_url, app_token, app_id, timeout=10):
        ...
        # raise exception if resp.success != True
        if not resp.get('success'):
            raise exceptions.AuthorizationError('get_session_token failed')
    ...
    def _get_challenge(self, base_url, timeout=10):
        ...
        # raise exception if resp.success != True
        if not resp.get('success'):
            raise exceptions.AuthorizationError('get_challenge failed')
    ...
    def _get_base_url(self, host, port, freebox_api_version):
        ...
        :return:
        '''
        baseUrl = 'http://{0}:{1}/api/{2}/'.format(host, port, freebox_api_version)
        print(baseUrl)
        return baseUrl
